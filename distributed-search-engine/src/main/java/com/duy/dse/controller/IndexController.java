...
public class IndexController {
     ...
     *  2018年10月9日
     */
	@PostMapping("batchmaintainIndex")
	public ResponseVO<?> batchmaintainIndex(@RequestBody IndexQuery maintainIndexQuery) throws IOException {

		indexService.batchmaintainIndex(maintainIndexQuery);
	...
	public ResponseVO<?> search(@RequestBody IndexSearchQuery indexSearchQuery) throws IOException, ParseException {
		...
		// 加入传入的list没有值，则不能做任何操作，直接返回
		if (indexSearchQuery.getContent() == null || indexSearchQuery.getContent().isEmpty()) {
			return ResponseVO.fail("传入的查询字段为空");
		}
		if (indexSearchQuery.getContentType() == null || indexSearchQuery.getContentType().isEmpty()) {
			return ResponseVO.fail("传入的内容类型为空");
		}
		
		List<IndexEntity> list = indexService.selectIndexDocmentByConetent(indexSearchQuery);
